...
export class GitHubProvider extends BaseProvider {
	...
	async #request(endpoint, options = {}) {
		...
		if (!response.ok) {
			...
			throw new ProviderError(
				...
				'API_ERROR',
				{ statusCode, retryable, provider: 'github' }
			);
		}
	...
	/**
	 * Fetch PRs matching a search query with full details (checks, reviews, etc.)
	 * Shared logic for getMyPullRequests and getReviewRequests.
	 * @param {string} query - GitHub search query string
	 * @returns {Promise<PullRequest[]>}
	 */
	async #fetchPRsWithQuery(query) {
		const data = await this.#request(`/search/issues?q=${query}&sort=updated&order=desc`);
			...
			items.map(async (issue) => {
				...
				try {
					...
					const [checks, reviews] = await Promise.all([
						this.getCheckStatus(repoFullName, sha).catch(() => ({ status: 'unknown', details: [] })),
	 ...
	 * @param {string} repoFullName - 'owner/repo' format
	 * @param {string} sha - Commit SHA to check status for
	 * @returns {Promise<Object>}
	 */
	async getCheckStatus(repoFullName, sha) {
		if (!sha) {
