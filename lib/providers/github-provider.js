...
/**
 * Validate that a URL is a safe HTTPS URL for API calls
...
function isValidApiUrl(url) {
	...
	try {
		...
		// Only allow HTTPS for API calls (security requirement)
...
export class GitHubProvider extends BaseProvider {
	constructor(config = {}) {
		...
		// Validate and sanitize baseUrl - only allow HTTPS
		// Note: Domain validation is intentionally flexible to support GitHub Enterprise instances.
		// Only protocol (HTTPS) is enforced to prevent SSRF with non-HTTP protocols.
		// For enterprise deployments, the baseUrl can point to any valid HTTPS endpoint.
		const providedBaseUrl = config.baseUrl || 'https://api.github.com';
		if (!isValidApiUrl(providedBaseUrl)) {
			// Fall back to default if invalid URL provided
	...
	async #request(endpoint, options = {}) {
		...
		if (endpoint.startsWith('http')) {
			...
			if (!isValidApiUrl(endpoint)) {
				throw new Error('Invalid API URL: only HTTPS URLs are allowed');
			}
			url = endpoint;
		} else {
