...
export class GitHubProvider extends BaseProvider {
	...
	async getCheckStatus(repoFullName, prNumber) {
		...
		const data = await this.#request(`/repos/${repoFullName}/commits/${sha}/check-runs`);
		...
		if (allComplete) {
			const allSuccess = details.every(d => d.conclusion === 'success');
			const anyFailure = details.some(d => d.conclusion === 'failure');
			status = allSuccess ? 'success' : anyFailure ? 'failure' : 'pending';
		}
