<script>
	...
	$: isConnected = !!(provider && provider.user && !reconnecting);
	$: jiraDashboardUrl = jiraUrl && isValidHttpUrl(jiraUrl) ? `${jiraUrl}/jira/for-you` : '';
	...
	<div class="page-shell mx-auto flex flex-col gap-4">
		...
		<SectionCard>
			...
			{#if isConnected}
				<div class="rounded-[20px] border border-soft bg-black/20 p-4">
					...
					<div class="mt-4">
						<Button variant="secondary" on:click={() => reconnecting = true}>Reconnect with a different token</Button>
					</div>
				</div>
			{:else}
				<div class="space-y-3">
					...
					<div class="flex flex-wrap gap-3">
						<Button on:click={validateAndSaveToken} disabled={validatingToken}>
							{validatingToken ? 'Validating...' : 'Connect'}
						</Button>
						{#if provider?.user}
							<Button variant="secondary" on:click={() => reconnecting = false}>Cancel</Button>
						{/if}
					</div>
					{#if tokenError}
		...
		<SectionCard>
			...
			<div class="grid gap-3 sm:grid-cols-2">
				<RadioCard name="pinnedTab" value="myPRs" currentValue={currentSettings.pinnedTab || 'myPRs'} title="My PRs" description="Track the pull requests you created and keep an eye on CI and review status." icon="📤" on:change={(event) => updatePinnedTab(event.detail)} />
				<RadioCard name="pinnedTab" value="toReview" currentValue={currentSettings.pinnedTab || 'myPRs'} title="To Review" description="Focus the popup on pull requests that need your attention as a reviewer." icon="📥" on:change={(event) => updatePinnedTab(event.detail)} />
			</div>
		</SectionCard>
		...
		<div class="grid gap-4 lg:grid-cols-2">
			...
			<SectionCard>
				...
				<div class="grid gap-3">
					<RadioCard name="displayMode" value="popup" currentValue={currentSettings.displayMode || 'popup'} title="Popup" description="Keep the toolbar interaction lightweight and compact." icon="📱" on:change={(event) => updateDisplayMode(event.detail)} />
					<RadioCard name="displayMode" value="fullpage" currentValue={currentSettings.displayMode || 'popup'} title="Full page" description="Use a larger tab surface for denser layouts and future filters." icon="🖥️" on:change={(event) => updateDisplayMode(event.detail)} />
				</div>
			</SectionCard>
		</div>
		...
		<SectionCard>
			...
			<div class="grid gap-3 sm:grid-cols-2">
				<RadioCard name="persistFilters" value={true} currentValue={currentSettings.persistFilters ?? true} title="Remember Filters" description="Keep your search and active filters across extension sessions forever." icon="💾" on:change={() => updateSetting('persistFilters', true)} />
				<RadioCard name="persistFilters" value={false} currentValue={currentSettings.persistFilters ?? true} title="Per Session" description="Clear search query and filters every time you close the popup." icon="🧹" on:change={() => updateSetting('persistFilters', false)} />
			</div>
		</SectionCard>
